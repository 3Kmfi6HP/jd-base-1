...
## Mod: https://github.com/3Kmfi6HP/jd-base-1
## Modified： 2021-03-12
## Version： alpha
# git remote set-url origin git@gitee.com:lxk0301/jd_scripts.git
# 写入 sshkey 以顺利 pull gitee 脚本
...
mkdir -p /root/.ssh && echo -e $KEY > /root/.ssh/id_rsa && chmod 600 /root/.ssh/id_rsa && ssh-keyscan gitee.com > /root/.ssh/known_hosts
...
#自定义添加脚本
listDIY=${Scripts2Dir}/crontab_diy.list
...
function Update_Cron {
  if [ -f ${ListCron} ]; then
    ...
    H="${RanHour},${ranH}"
    #每天5-23随机分钟更新
    perl -i -pe "s|.+(bash ${ShellDir}/git_pull2.+)|${RanMin} 5-23 \* \* \* sleep ${RanSleep} && \1|" ${ListCron}
    #美丽研究院分随机cron
